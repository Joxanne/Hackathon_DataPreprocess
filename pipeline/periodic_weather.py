...
from config import CWA_API_KEY, EMBED_MODEL, PG_DASHBOARD, QDRANT, VECTOR_SIZE

# ── 資料集設定 ─────────────────────────────────────────────────────────────────
QDRANT_COLLECTION = "weather_station_tpe"
...
def save_to_qdrant(records: list[dict]) -> None:
    ...
    def _fmt(r: dict) -> str:
        temp = f"{r['temperature']}°C" if r["temperature"] is not None else "溫度未知"
        humid = f"{r['humidity']}%" if r["humidity"] is not None else ""
        desc = r["weather_desc"] or "天氣未知"
        return (
            f"passage: 氣象站：{r['station_name']}，"
            f"位於{r['county']}{r['town']}，"
            f"目前{desc}，氣溫{temp}，濕度{humid}"
        )

    texts = [_fmt(r) for r in records]
    ...
    points = [
        PointStruct(
            id=str(uuid.uuid5(uuid.NAMESPACE_DNS, r["station_id"])),
            vector=v.tolist(),
            payload={
                ...
                "pressure":          r["pressure"],
                "precipitation_now": r["precipitation_now"],
